...
export function generateTradeFlowReport({ flows, exporter, importer, layer }) {
  ...
  const subtitle=!layer||layer==='ALL'?'L1 + L2 FLOWS OVERVIEW':layer==='L1'?'L1 PRIMARY FLOWS ONLY':'L2 VALUE-ADDED FLOWS ONLY';

  const row=(f,i)=>`<tr style="background:${f.layer==='L2'?'rgba(74,158,218,0.08)':i%2===0?'rgba(255,255,255,0.02)':'transparent'}">
    <td style="color:#8a9ab5;font-size:10px;padding:5px 4px;white-space:nowrap;width:20px">${i+1}</td>
    <td style="color:#fff;font-size:10px;padding:5px 4px;font-weight:600;max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${f.product||''}</td>
    <td style="color:#8a9ab5;font-size:10px;padding:5px 4px;white-space:nowrap">${f.importer||''}</td>
    <td style="padding:5px 4px"><span style="padding:1px 5px;border-radius:2px;font-size:8px;font-weight:700;background:${f.layer==='L2'?'rgba(74,158,218,0.2)':'rgba(46,204,113,0.2)'};color:${f.layer==='L2'?'#4a9eda':'#2ecc71'}">${f.layer||'L1'}</span></td>
    <td style="color:#c8993a;font-size:10px;padding:5px 4px;font-weight:700;white-space:nowrap">${fmt(f.fob_usd)}</td>
    <td style="color:#ccc;font-size:10px;padding:5px 4px;white-space:nowrap">${f.volume_mt?f.volume_mt.toLocaleString(undefined,{maximumFractionDigits:0}):'-'}</td>
    <td style="color:#ccc;font-size:10px;padding:5px 4px;white-space:nowrap">${f.price_per_kg?'$'+f.price_per_kg.toFixed(2):'-'}</td>
    <td style="color:${f.importer_is_processor?'#e8b84b':'#4a5a70'};font-size:10px;padding:5px 4px">${f.importer_is_processor?'YES':'-'}</td>
  </tr>`;

  const mapFlows=JSON.stringify(flows.map(f=>({importer:f.importer,fob_usd:f.fob_usd||0,layer:f.layer||'L1'})));
  const africaPos=JSON.stringify({"Algeria":[0.42,0.12],"Egypt":[0.72,0.18],"Morocco":[0.18,0.12],"Libya":[0.55,0.18],"Tunisia":[0.45,0.10],"South Africa":[0.55,0.88],"Nigeria":[0.42,0.48],"Kenya":[0.72,0.55],"Ghana":[0.33,0.50],"Senegal":[0.12,0.40],"Ivory Coast":[0.28,0.52],"Cameroon":[0.48,0.53],"Tanzania":[0.68,0.63],"Ethiopia":[0.72,0.43],"Angola":[0.48,0.70],"Mozambique":[0.65,0.72],"Zambia":[0.58,0.68],"Zimbabwe":[0.60,0.75],"Uganda":[0.65,0.50],"Dem. Rep. of the Congo":[0.52,0.60],"Congo":[0.48,0.58],"Gabon":[0.44,0.57],"Mali":[0.30,0.32],"Niger":[0.42,0.32],"Burkina Faso":[0.32,0.40],"Togo":[0.36,0.48],"Benin":[0.38,0.46],"Mauritius":[0.85,0.72],"Cabo Verde":[0.05,0.30],"Djibouti":[0.78,0.42],"Guinea":[0.15,0.45],"Liberia":[0.18,0.52]});
...
body{background:#0b1929;color:#fff;font-family:'IBM Plex Mono',monospace;width:1050px;margin:0 auto}
table{border-collapse:collapse;width:100%}
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
</style></head><body>
<div style="width:1050px;padding:32px;background:#0b1929">

  <!-- HEADER: 3 columns -->
  <div style="display:table;width:100%;margin-bottom:24px;padding-bottom:20px;border-bottom:2px solid rgba(200,153,58,0.4)">
    <div style="display:table-cell;vertical-align:middle;width:200px">
      <div style="display:flex;align-items:center;gap:6px">
        <span style="font-family:'Syne',sans-serif;font-weight:800;font-size:30px;color:#fff">JMR</span>
        <div>
          <svg width="72" height="14" viewBox="0 0 72 14">
            <path d="M0 7 Q36 0 72 7" fill="none" stroke="#c8993a" stroke-width="2.5" stroke-linecap="round"/>
            <circle cx="0" cy="7" r="3" fill="#c8993a"/>
            <circle cx="72" cy="7" r="3" fill="#fff"/>
          </svg>
          <div style="font-family:'Syne',sans-serif;font-weight:500;font-size:14px;color:#c8993a;letter-spacing:2px">GLOBAL</div>
        </div>
      </div>
      <div style="font-size:9px;letter-spacing:0.2em;color:#c8993a;margin-top:4px">GLOBAL INGREDIENTS</div>
    </div>
    <div style="display:table-cell;vertical-align:middle;text-align:center">
      <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:26px;color:#fff;letter-spacing:0.04em;text-transform:uppercase;line-height:1.2">${title}</div>
      <div style="font-size:11px;color:#c8993a;letter-spacing:0.18em;margin-top:8px;text-transform:uppercase">${subtitle}</div>
    </div>
    <div style="display:table-cell;vertical-align:middle;width:110px;text-align:right">
      <div style="display:inline-block;background:#c8993a;border-radius:10px;padding:12px 16px;text-align:center">
        <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:40px;color:#0b1929;line-height:1">${flows.length}</div>
        <div style="font-size:8px;color:#0b1929;letter-spacing:0.08em;margin-top:4px;font-weight:700">TOTAL FLOWS<br>${!layer||layer==='ALL'?'L1 + L2':layer}</div>
      </div>
    </div>
  </div>

  <!-- STAT BOXES -->
  <div style="display:table;width:100%;margin-bottom:24px;border-spacing:12px 0">
    ${[
      {icon:'💰',label:'TOTAL FOB VALUE',val:fmt(totalFob),unit:'USD'},
      {icon:'⚓',label:'TOTAL VOLUME',val:(totalVol/1e6).toFixed(2)+'M',unit:'MT'},
      {icon:'📊',label:'WEIGHTED AVG PRICE',val:'$'+avgPrice.toFixed(2),unit:'/KG'},
      {icon:'🏭',label:'FLOWS TO PROCESSORS',val:processors,unit:'YES ⚡'},
    ].map(s=>`<div style="display:table-cell;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:16px;width:25%">
      <div style="font-size:22px;margin-bottom:8px">${s.icon}</div>
      <div style="font-size:8px;letter-spacing:0.14em;color:#8a9ab5;margin-bottom:6px">${s.label}</div>
      <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:28px;color:#c8993a;line-height:1">${s.val}</div>
      <div style="font-size:10px;color:#8a9ab5;margin-top:4px">${s.unit}</div>
    </div>`).join('<div style="display:table-cell;width:12px"></div>')}
  </div>

  <!-- MAP + RANKINGS: side by side using table -->
  <div style="display:table;width:100%;margin-bottom:24px;border-spacing:0">
    <div style="display:table-cell;width:460px;vertical-align:top;padding-right:20px">
      <div style="background:rgba(255,255,255,0.03);border-radius:10px;overflow:hidden">
        <div style="padding:14px 16px;background:rgba(200,153,58,0.06);border-bottom:1px solid rgba(200,153,58,0.12)">
          <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:13px;color:#fff;margin-bottom:4px">STRONG TRADE CONNECTIONS</div>
          <div style="font-size:10px;color:#8a9ab5">Key agricultural & ingredient flows from ${exporter||'Mercosur'} to markets across Africa.</div>
        </div>
        <canvas id="map-canvas" width="440" height="280" style="display:block"></canvas>
        <div style="display:flex;gap:16px;padding:10px 16px">
          <div style="display:flex;align-items:center;gap:6px;font-size:10px;color:#8a9ab5"><div style="width:10px;height:10px;border-radius:50%;background:#2ecc71"></div>L1 – Primary</div>
          <div style="display:flex;align-items:center;gap:6px;font-size:10px;color:#8a9ab5"><div style="width:10px;height:10px;border-radius:50%;background:#4a9eda"></div>L2 – Value Added</div>
        </div>
      </div>
    </div>
    <div style="display:table-cell;vertical-align:top">
      <!-- Top Markets -->
      <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:16px;margin-bottom:16px">
        <div style="font-size:9px;letter-spacing:0.16em;color:#c8993a;margin-bottom:14px;padding-bottom:8px;border-bottom:1px solid rgba(200,153,58,0.2)">TOP 5 MARKETS BY FOB VALUE</div>
        ${topMarkets.map(([name,val],i)=>`
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <div style="font-size:11px;color:#8a9ab5;width:16px">${i+1}</div>
          <div style="font-size:10px;font-weight:700;color:#c8993a;width:24px">${name.slice(0,2).toUpperCase()}</div>
          <div style="flex:1;font-size:12px;color:#fff">${name}</div>
          <div style="width:90px;background:rgba(255,255,255,0.08);border-radius:2px;height:5px"><div style="width:${(val/topMarkets[0][1]*100).toFixed(0)}%;background:#c8993a;height:5px;border-radius:2px"></div></div>
          <div style="font-size:11px;color:#c8993a;font-weight:700;width:56px;text-align:right">${fmt(val)}</div>
          <div style="font-size:10px;color:#8a9ab5;width:34px;text-align:right">${(val/totalFob*100).toFixed(1)}%</div>
        </div>`).join('')}
      </div>
      <!-- Top Products -->
      <div style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:10px;padding:16px">
        <div style="font-size:9px;letter-spacing:0.16em;color:#c8993a;margin-bottom:14px;padding-bottom:8px;border-bottom:1px solid rgba(200,153,58,0.2)">TOP PRODUCTS BY FOB VALUE</div>
        ${topProducts.map(([name,val])=>`
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px">
          <div style="width:8px;height:8px;border-radius:50%;background:#c8993a;flex-shrink:0"></div>
          <div style="flex:1;font-size:12px;color:#fff">${name}</div>
          <div style="font-size:12px;color:#4a9eda;font-weight:700">${fmt(val)}</div>
          <div style="font-size:10px;color:#8a9ab5;width:36px;text-align:right">${(val/totalFob*100).toFixed(1)}%</div>
        </div>`).join('')}
      </div>
    </div>
  </div>

  <!-- DETAIL TABLE: two columns using table-cell -->
  <div style="font-size:9px;letter-spacing:0.16em;color:#c8993a;margin-bottom:10px;text-transform:uppercase">Trade Flows Detail (${!layer||layer==='ALL'?'L1 + L2':layer})</div>
  <div style="display:table;width:100%;margin-bottom:24px">
    <div style="display:table-cell;vertical-align:top;width:50%;padding-right:10px">
      <table style="width:100%;border-collapse:collapse;table-layout:fixed">
        <colgroup><col style="width:22px"><col style="width:110px"><col style="width:90px"><col style="width:34px"><col style="width:58px"><col style="width:62px"><col style="width:44px"><col style="width:34px"></colgroup>
        <thead><tr style="background:rgba(200,153,58,0.12)">
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">#</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">PRODUCT</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">TO</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">L</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">FOB</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">VOL MT</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">$/KG</th>
          <th style="color:#c8993a;padding:6px 4px;text-align:left;font-size:8px;letter-spacing:0.08em;border-bottom:1px solid rgba(200,153,58,0.25)">PROC</th>
        </tr></thead>
        <tbody>${col1.map((f,i)=>row(f,i)).join('')}</tbody>
      </table>
    </div>
    <div style="display:table-cell;vertical-align:top;width:50%;padding-left:10px">
      <table style="width:100%;border-collapse:collapse;table-layout:fixed">
        ...
        <tbody>${col2.map((f,i)=>row(f,i+half)).join('')}</tbody>
      </table>
    </div>
  </div>
  ...
  <div style="display:table;width:100%;margin-bottom:24px;border-spacing:0">
    ${[
      {icon:'🌍',title:'North Africa leads',text:`${topMarkets.slice(0,3).map(m=>m[0]).join(', ')} account for ${(topMarkets.slice(0,3).reduce((s,m)=>s+m[1],0)/totalFob*100).toFixed(1)}% of total FOB value.`},
      {icon:'🌾',title:`${topProducts[0]?.[0]||''} dominates`,text:`${topProducts[0]?.[0]||''} represents ${topProducts[0]?(topProducts[0][1]/totalFob*100).toFixed(1):0}% of total trade value.`},
      {icon:'🏭',title:'Value added',text:`${flows.filter(f=>f.layer==='L2').length} L2 flows across ${[...new Set(flows.filter(f=>f.layer==='L2').map(f=>f.importer))].length} markets.`},
      {icon:'🤝',title:'Diverse reach',text:`${flows.length} unique flows across ${[...new Set(flows.map(f=>f.importer))].length} African countries.`},
    ].map(t=>`<div style="display:table-cell;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:8px;padding:14px;vertical-align:top">
      <div style="font-size:24px;margin-bottom:8px">${t.icon}</div>
      <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:12px;color:#fff;margin-bottom:5px">${t.title}</div>
      <div style="font-size:10px;color:#8a9ab5;line-height:1.6">${t.text}</div>
    </div><div style="display:table-cell;width:12px"></div>`).join('')}
  </div>

  <!-- FOOTER -->
  <div style="display:flex;justify-content:space-between;align-items:center;padding-top:16px;border-top:1px solid rgba(255,255,255,0.08);font-size:10px;color:#8a9ab5">
...
(function(){
  ...
  const W=440,H=280,pad={l:100,r:10,t=10,b:10};
  ...
  ctx.closePath();ctx.fillStyle='#1a3460';ctx.fill();ctx.strokeStyle='#2a4a7f';ctx.lineWidth=1;ctx.stroke();
  const argX=45,argY=H*0.58;
  ctx.beginPath();ctx.arc(argX,argY,14,0,Math.PI*2);ctx.fillStyle='rgba(200,153,58,0.2)';ctx.fill();ctx.strokeStyle='#c8993a';ctx.lineWidth=2;ctx.stroke();
  ctx.fillStyle='#c8993a';ctx.font='bold 9px monospace';ctx.textAlign='center';ctx.fillText('${(exporter||'ARG').slice(0,3).toUpperCase()}',argX,argY+3);
  const POS=${africaPos};
  const flows=${mapFlows};
  const maxFob=Math.max(...flows.map(f=>f.fob_usd||0),1);
  [...new Set(flows.map(f=>f.importer))].forEach(dest=>{
    const pos=POS[dest];if(!pos)return;
    const df=flows.filter(f=>f.importer===dest);
    const total=df.reduce((s,f)=>s+(f.fob_usd||0),0);
    const px=pad.l+pos[0]*mapW,py=pad.t+pos[1]*mapH;
    const w=Math.max(0.5,Math.min(3.5,total/maxFob*3.5));
    const hasL2=df.some(f=>f.layer==='L2');
    ctx.beginPath();ctx.moveTo(argX,argY);
    ctx.quadraticCurveTo((argX+px)/2,Math.min(argY,py)-50,px,py);
    ctx.strokeStyle=hasL2?'rgba(74,158,218,0.7)':'rgba(200,153,58,0.7)';ctx.lineWidth=w;ctx.stroke();
    ctx.beginPath();ctx.arc(px,py,Math.max(3,w*1.8),0,Math.PI*2);ctx.fillStyle=hasL2?'#4a9eda':'#2ecc71';ctx.fill();
  });
})();
</script>
</body></html>`;
